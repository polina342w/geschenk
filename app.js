...
document.querySelector('.peek').addEventListener('click',event=>{const input=document.querySelector('#password'),visible=input.type==='password';input.type=visible?'text':'password';event.currentTarget.classList.toggle('is-visible',visible);event.currentTarget.setAttribute('aria-label',visible?'Hide password':'Show password')});
